...
class Helpers
{
  ...
  {
    // Cek pada tabel settings_web untuk logo_website dan nama_website
    $settings = DB::table('settings_web')->first();

    $logo_website = $settings->logo_website ?? null;
    $nama_website = $settings->nama_website ?? 'ADMIN'; // Default jika nama_website tidak ada

    // Kembalikan data dalam bentuk array atau objek sesuai kebutuhan
    return [
      'logo_website' => $logo_website ? asset('storage/' . $logo_website) : null, // Ganti dengan path logo_website yang sesuai
      'nama_website' => $nama_website
    ];
  }

  public static function berandaUser()
  {
    // Cek pada tabel settings_web untuk logo_website dan nama_website
    $settings = DB::table('settings_web')->first();

    $logo_website = $settings->logo_website ?? null;
    $nama_website = $settings->nama_website ?? 'WEBSITE';
    $deskripsi = $settings->deskripsi_website ?? '-';
    $keyword = $settings->keyword_website ?? '-';

    return [
      'logo_website' => $logo_website ? asset('storage/' . $logo_website) : null,
      'nama_website' => $nama_website,
      'deskripsi_website' => $deskripsi,
      'keyword_website' => $keyword,
    ];
  }
}
